"""The τ-bench airline tools, ported.

Ported rather than rewritten, and ported rather than imported. Rewritten, a tool that
behaves differently from the original would make any figure this repo publishes
incomparable to published τ-bench results — the reason #23 says the same thing about the
cctx classifiers. Imported, the τ-bench package is git-install-only and pulls `openai`,
`mistralai`, `anthropic`, `google-generativeai` and `litellm` into a project whose
dependency allowlist is four packages.

Each class body below is upstream's, unchanged except for the base class, which is defined
here so there is no import to satisfy. `invoke` mutates the database dict it is handed, so
callers hand it a copy per run — see `agent.py`.

Source: https://github.com/sierra-research/tau-bench,
tau_bench/envs/airline/tools/, MIT, Copyright (c) 2024 Sierra. See NOTICE.
"""
...
class Tool(abc.ABC):
    """Upstream's `tau_bench.envs.tool.Tool`, so the ported bodies need no rewriting."""
...
#: Every tool by the name the model calls it by, built from each class's own
#: `get_info()` so the registry cannot drift from the schema the model is shown.
TOOLS: dict[str, type[Tool]] = {
    BookReservation.get_info()["function"]["name"]: BookReservation,
    Calculate.get_info()["function"]["name"]: Calculate,
    CancelReservation.get_info()["function"]["name"]: CancelReservation,
    GetReservationDetails.get_info()["function"]["name"]: GetReservationDetails,
    GetUserDetails.get_info()["function"]["name"]: GetUserDetails,
    ListAllAirports.get_info()["function"]["name"]: ListAllAirports,
    SearchDirectFlight.get_info()["function"]["name"]: SearchDirectFlight,
    SearchOnestopFlight.get_info()["function"]["name"]: SearchOnestopFlight,
    SendCertificate.get_info()["function"]["name"]: SendCertificate,
    Think.get_info()["function"]["name"]: Think,
    TransferToHumanAgents.get_info()["function"]["name"]: TransferToHumanAgents,
    UpdateReservationBaggages.get_info()["function"]["name"]: UpdateReservationBaggages,
    UpdateReservationFlights.get_info()["function"]["name"]: UpdateReservationFlights,
    UpdateReservationPassengers.get_info()["function"]["name"]: UpdateReservationPassengers,
}


#: The schemas, in the shape the Anthropic messages API takes them.
def schemas() -> List[Dict[str, Any]]:
    return [
        {
            "name": info["function"]["name"],
            "description": info["function"]["description"],
            "input_schema": info["function"]["parameters"],
        }
        for info in (cls.get_info() for cls in TOOLS.values())
    ]
